...
const timeRandomizer = (start, end) =>
...
const gameDialogue = {
  ...
  intro: `His garden has been overrun by Digletts. Try to hit at least eight before they run away!`,
  instructions: `Point and click to hit the digletts as they pop out of their holes.`,
...
$(() => {
  ...
  const game = () => {
    ...
    setTimeout(() => {
      ...
      if (p1Score < 5) {
        ...
        playSong(winSound);
        $endMsg.text(
          `Nice job! You managed to run off ${p1Score} of 'em. Now the professor can finally get his garden back in order.`
        );
      }
      return;
    }, 16000);
  ...
  const gameMode = (event) => {
    ...
    $(event.currentTarget).hasClass("twoplayer")
      ? (twoPlayers = true)
      : (twoPlayers = false);
